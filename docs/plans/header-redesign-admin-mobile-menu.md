...
- [x] `MenuModal` に管理者ページリンクを表示する。
- [x] 非管理者の場合は、メニューが開いても管理者リンクを表示しない。
- [x] メニューリンク押下時に既存通りメニューを閉じる。
- [x] PC へ切り替わった時に既存通りメニューを閉じる。
...
- [x] PC のヘッダーを添付画像に近い黒系背景・ロゴ左寄せ・タブ中央寄せに調整する。
- [x] 管理者アイコンリンク、ユーザー情報、ログアウトアイコンの配置を整理する。
- [x] hover で大きく赤背景になる現状挙動を残すか削除するか、デザインに合わせて判断する。
