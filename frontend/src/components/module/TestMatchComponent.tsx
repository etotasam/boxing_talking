...
export const TestMatchComponent = ({ match, className, onClick = () => null }: Props) => {
  enum MatchIs {
    TODAY = "text-red-600",
    PAST = "text-gray-400",
  ...
  useEffect(() => {
    ...
    const ratioRed = Math.round(red);
    const ratioBlue = Math.round(blue);
    setRatioRed(`${ratioRed}%`);
    setRatioBlue(`${ratioBlue}%`);
  }, [match]);
  ...
  return (
    ...
    >
      ...
      <div onClick={() => onClick(match.id)} className="flex">
        <TestFighter fighter={match.red} cornerColor={"red"} className={"w-1/2"} />
        <TestFighter fighter={match.blue} cornerColor={"blue"} className={"w-1/2"} />
