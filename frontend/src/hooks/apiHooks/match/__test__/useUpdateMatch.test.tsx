...
import { createWrapper, matchesResponse, updateMatchInput } from './testUtils';
...
describe('useUpdateMatch', () => {
  ...
  test('更新成功時に試合データを再取得し、成功トーストを表示する', async () => {
    ...
    act(() => {
      result.current.updateMatch(updateMatchInput);
    });

    await waitFor(() => {
      expect(mocks.showFullScreenLoading).toHaveBeenCalledTimes(1);
      expect(mocks.patch).toHaveBeenCalledTimes(1);
      expect(mocks.patch).toHaveBeenCalledWith(API_PATH.MATCH, updateMatchInput);
      expect(mocks.hideFullScreenLoading).toHaveBeenCalledTimes(1);
      expect(mocks.showSuccessToast).toHaveBeenCalledTimes(1);
      expect(mocks.showSuccessToast).toHaveBeenCalledWith(MESSAGE.MATCH_UPDATE_SUCCESS);
      expect(mocks.get).toHaveBeenCalledTimes(2);
      expect(mocks.showErrorToast).not.toHaveBeenCalled();
    });
  });

  test('更新失敗時は失敗トーストを表示し、試合データを再取得しない', async () => {
    vi.mocked(Axios.patch).mockRejectedValueOnce(new Error('failed'));
    ...
    await waitFor(() => {
      ...
      expect(mocks.showErrorToast).toHaveBeenCalledWith(MESSAGE.MATCH_UPDATE_FAILED);
