...
- 役割を分ける場合は、以下の流れを基本とする:
  ...
  2. 設計が必要な場合は Planner が Explorer の調査結果を踏まえて設計すること
     比較的大きい変更では、承認後に Plan Document を作成すること
     Plan Document の作成基準、保存先、書式は `.agents/skills/planner/SKILL.md` に従うこと
  3. 重要な変更、複数ファイルにまたがる変更、Plan Document を作成する規模の変更では、Planner の設計案を実装前に Reviewer subagent が評価すること
     - 仕様の抜け漏れ
     - 影響範囲の妥当性
     - 実装方針の妥当性
     - テスト方針の妥当性
     - リグレッションリスク
     を確認すること
  4. 実装は承認後に Worker が実装すること
  5. テスト追加・更新が必要な場合は Tester が作成すること
  6. 実装後は Reviewer subagent が差分・仕様・テスト観点で確認すること
  7. 最後に関連テストを実行し、結果を報告すること
...
- `.env.production` および本番環境の秘密情報を含む設定ファイルは参照しないこと
- 必要な場合は理由を説明し、承認を得てから参照すること
...
- `rm -rf`、`rm -fr` および同等の再帰的強制削除は禁止する
- ファイル削除には `apply_patch` を使用する
- ディレクトリ削除が必要な場合は、対象と理由を提示して事前承認を得る
